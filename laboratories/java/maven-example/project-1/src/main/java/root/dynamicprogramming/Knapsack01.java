...
// 每件物品只能使用一次，计算将哪些物品装入背包里物品价值总和最大。
//
...
// case 2: dp[i - 1][j - weight[i]] + value[i]，选array[i]
// dp[i][j] = max { dp[i - 1][j], dp[i - 1][j - weight[i]] + value[i] }
// 从上面可以看出第i行的dp只会依赖第i-1行的dp
// dp可以改为2行
// dp可以改为1行，但遍历需要从大到小，否则会覆盖i-1的数据
    ...
    private static int doDirtyWork1(int[] weight, int[] value, int limit) {
        ...
        for (int i = 1; i < weight.length; ++ i) {
            for (int j = 1; j <= limit; ++ j) {
                ...
                int value2 = 0;
                if (weight[i] < j) {
                    value2 = dp[i - 1][j - weight[i]] + value[i];
                }
                if (value1 < value2) {
                    dp[i][j] = value2;
                } else {
    ...
    private static int doDirtyWork2(int[] weight, int[] value, int limit) {
        ...
        for (int i = 1; i < weight.length; ++ i) {
            for (int j = 1; j <= limit; ++ j) {
                ...
                int value2 = 0;
                if (weight[i] < j) {
                    value2 = dp[index1][j - weight[i]] + value[i];
                }
                if (value1 < value2) {
                    dp[index2][j] = value2;
                } else {
